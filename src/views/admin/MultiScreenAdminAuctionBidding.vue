<template>
  <div class="content-holder-p-0" :class="{ 'sidebar-open': mbidSidebar }">
    ...
    <!-- Join Auction Dialog -->
    <Dialog
      v-model:visible="auctionPasswordModal"
      modal
      :header="`Enter Auction ${auction?.vsAuctionCode} Password`"
    >
      <form  @submit.prevent="submitAuctionPassword()">
        <div  v-if=isWatcherPasscodeEnabled class="flex align-items-center gap-3 mb-5">
    ...
    <div
      class="mbid-sidebar fixed top-0 left-0 bottom-0 w-[var(--mbid-sidebar-width)] bg-slate-600"
    >
      <div class="mbid-sidebar-content">
        <div class="px-6 py-6">
          ...
          <div class="my-2 text-center">
            <p
              class="px-4 py-1.5 inline-block rounded text-sm font-normal text-center text-sky-600 bg-sky-100"
            >
              You can join atmost {{ maxAuctionJoinLimit }} auctions
            </p>
          </div>
          <div class="fm-group">
            <div class="fm-inner">
              <InputText v-model="filter" placeholder="Search Auction Code" />
              <magnifyingglass
                class="fm-icon fm-prefix"
                @click="fetchScheduledAuctionsBidder()"
              ></magnifyingglass>
            </div>
          </div>
          <!-- <InputGroup>
        ...
        <div class="mbid-sidebar-card-holder px-6 pb-5">
          <div
            class="card card-auction mb-4 last:mb-0"
            v-for="auction in scheduledAuctions"
            :key="auction.pklAuctionId"
          >
            <div class="ca-title text-xl font-bold text-[var(--neutral-600)]">
              {{ auction.vsAuctionCode }}
            </div>
            <div
              class="ca-subtitle mb-3 text-sm text-[var(--neutral-400)] line-clamp-1"
            >
              {{ auction.vsAuctionDescription }} and something else that we can
              add
            </div>
            <div class="ca-info mb-3 text-[var(--neutral-400)]">
              <div class="ca-info-item mb-1">
                ...
                <div class="font-medium">
                  {{
                    moment(auction.dtStartDate).format("DD/MM/YYYY hh:mm:ss A")
                  }}
                </div>
              </div>
              <div class="ca-info-item">
                ...
                <div class="font-medium">
                  {{
                    moment(auction.dtEndDate).format("DD/MM/YYYY hh:mm:ss A")
                  }}
                </div>
              </div>
            </div>
            <div class="ca-action text-center">
              <Button
                class="btn-sm w-2/3"
                :disabled="isAuctionJoined(auction.pklAuctionId)"
                @click="joinAuction(auction)"
                >{{
                  isAuctionJoined(auction.pklAuctionId) ? "Joined" : "Join"
                }}</Button
              >
            </div>
          </div>
    ...
    <div class="mbid-main-content ml-[var(--mbid-sidebar-width)]">
      <div
        class="mbid-header px-8 h-[var(--mbid-header-height)] flex items-center"
      >
        <button
          class="btn btn-mbid-sidebar-toggle mr-4 w-[55px] [&_svg]:h-[28px]"
          @click="mbidSidebar = !mbidSidebar"
        >
          <fa-xmark
            v-if="mbidSidebar"
            class="fill-[var(--neutral-400)]"
          ></fa-xmark>
          <fa-bars v-else class="fill-[var(--neutral-400)]"></fa-bars>
        </button>
        <div
          class="py-2 px-8 flex gap-x-2 items-center bg-red-700 bg-opacity-10 rounded-md border border-red-700 border-opacity-40 text-center"
        >
          <div class="text-sm text-red-700">Server Time:</div>
          <div class="text-[1.75rem] leading-[2rem] font-semibold text-red-700">
            {{ latestTime }}
          </div>
        </div>
        <div class="ml-auto flex items-center">
          ...
          <router-link
            to="/UserDashboard"
            class="btn btn-mbid-back ml-5 border border-slate-200 bg-white"
          >
            <fa-chevron-left></fa-chevron-left>
            <div class="text" @click="dashboard">
              <span
                class="-mb-1 block text-xs leading-[1] text-[var(--neutral-200)]"
                >back to</span
              >
              <span class="text-sm leading-[1] text-[var(--neutral-300)]"
                >Dashboard</span
              >
            </div>
          </router-link>
      ...
      <div class="mbid-auction-holder px-8 pb-7">
        <div class="auction-card-container">
          <WatcherAuctionCard
            ...
            v-for="auction in auctionStore.auctionList"
            :key="auction.auctionId"
            :auction="auction"
            :latestTime="latestTime"
            :active-index="activeAuction"
            :clientLoginIpAddress="clientLoginIpAddress"
            @activeTab="activeTab"
          />
          <WatcherAuctionCard
            class="mb-6 last:mb-0"
...
function isAuctionJoined(auctionId_) {
  return (
    auctionStore.auctionList?.findIndex(
      ({ auctionId }) => auctionId == auctionId_
    ) > -1
  );
}
...
//       auctionUserId: auction.value.auctionUserId,
//     });
//   }
...
const submitAuctionPassword = async () => {
  if (!isWatcherPasscodeEnabled.value) {
    // If watcher passcode is not enabled, open the modal for password entry
    await fetchWatcherPasscodeDetails()
 
console.log("!isWatcherPasscodeEnabled.value",!isWatcherPasscodeEnabled.value)
  } else {
    // Handle the usual passcode-enabled scenario
...
function checkPasscode() {
  return new Promise((resolve) => {
    ...
    new MQL()
      ...
      .setData({
        auctionId: auction.value.pklAuctionId,
        auctionPassword: auctionPassword.value,
      ...
      .then((rs) => {
        ...
        if (rs.isValid("CheckAuctionPassword")) {
          if (res.result.auctionPassword.checkAuctionPassword != 0) {
            ...
            auction.value.auctionUserId =
              res.result.auctionPassword.auctionUserId;
            toast.add({
              severity: "success",
              summary: "Correct Password",
              detail: "Welcome to Auction Live",
              life: 3000,
            });
            resolve(true);
          }
...
function syncTime() {
  axios
    .get("/bidding-server-http/o/time")
    .then((res) => {
      // internetVisible.value = false
      latestTime.value = moment(res.data.ServerTime).format(
...
function joinAuction(auction_) {
  ...
  v$.value.$reset()
    ...
    toast.add({
      severity: "error",
      summary: "Oops",
      detail: `Can not join more than ${maxAuctionJoinLimit.value} auctions`,
      life: 3000,
    });
  }
        }
...
.content-holder.content-holder-p-0 {
  padding: 0;
}

.btn-mbid-sidebar-toggle {
  padding-inline: 15px;
}
.btn-mbid-back {
  ...
  &:hover {
    ...
    background: rgb(var(--primary-400) / 0.5);
    svg {
...
.mbid-sidebar-card-holder {
  height: calc(100vh - 172px);
  overflow-y: auto;
}
.mbid-auction-holder {
  height: calc(100vh - var(--mbid-header-height));
  overflow-y: auto;
}
...
.table-auction {
  font-size: 15px;
  border: 2px solid theme("colors.sky[400]"); /* box-shadow: 0 10px 40px 0 rgba(0, 0, 0, 0.1);*/
  th,
  td {
    padding: 12px 15px;
  ...
  thead {
    tr {
      th {
        background: lighten(#e2e8f0, 3%) /*theme('colors.slate[200]')*/;
      }
      &:nth-child(3) {
        th {
  ...
  .fm-control.fm-select {
    padding-block: 7px;
    padding-inline: 10px;
    width: auto;
    height: var(--fm-height-sm);
  }
  .btn {
    white-space: nowrap;
  }
...
.details-holder {
  ...
  line-height: 24px;
  text-align: left;
  box-shadow: inset 0 1px 0 0 theme("colors.slate[300]");
