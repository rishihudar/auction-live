<template>
    <div>
        ...
        <Dialog v-model:visible="leaveAuctionVisible" modal header="Are you sure you want to leave?"
            :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #default>
                Are you sure you want to leave <strong>{{ auctionDetails.auctionCode }}</strong>?
        ...
        <Dialog v-model:visible="highestBidderVisible" modal header="Highest Bidder" :style="{ width: '50rem' }">
            <div>
                <strong class="text-center">
                    You are the highest bidder. For Auction {{ auctionDetails.auctionCode }}
        ...
        <Dialog v-model:visible="propertyVisible" maximizable modal header="Item Selection" :style="{ width: '50rem' }"
        ...
        <table class="table table-auction bg-white" :class="{ 'show-details': showDetails }">
            <thead>
                <tr>
                    <th colspan="5">
                        <div class="ta-header flex items-center">
                            <div class="ta-header-title text-2xl font-bold text-[var(--neutral-600)]">Auction Code: {{ auctionDetails.auctionCode }}</div>
                ...
                <tr>
                    <th colspan="5">
                        <div class="ta-subtitle flex items-center">
                            <div class="ta-subtitle-title text-base font-semibold text-[var(--neutral-600)]">{{ properties[0]?.vsInventoryCategoryName }}: {{ properties[0]?.inventoryHierarchy }}</div>
                            <div class="ta-subtitle-info ml-auto text-base text-[var(--neutral-600)]">Round: {{ auctionDetails.currentRound }} of {{ auctionDetails.numberOfRounds }}</div>
                        </div>
                    </th>
                </tr>
                <tr>
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <div class="flex flex-col align-items-center gap-2 items-center">
                            <div class="font-black" v-if="bidFlag && !roundHasEnded && count">
                                <span>
                                    {{ currencyFormat(bidValue) }}
                                </span>
                            </div>
                            <div class="flex gap-1 items-center">
                                <!-- <div v-if="bidFlag && !roundHasEnded"> -->
                                    <Dropdown v-show="bidFlag && !isbidDisabled" v-model="count" :options="multiplieries" optionLabel="text"
                                        optionValue="value" placeholder="Pick" />
                                <!-- </div> -->
                                <Button v-if="!bidFlag || roundHasEnded" :disabled="isbidDisabled" @click="bidFlag = true" class="btn-sm">Bid</Button>
                                <!-- <Button v-if="bidFlag && !roundHasEnded" :disabled="!count" @click="bidPlaced()" class="btn-sm">Place Bid</Button> -->
                            </div>
                        </div>
                    </td>
                    <td><span class="time-holder" ref="timeLeft"></span></td>
                    <td>
                        <div class="flex flex-wrap flex-col gap-1 items-center justify-center">
                            <!-- <Button label="Item Selection" :disabled="isItemSelectionBtnDisable" @click="openItem()" class="btn-sm"></Button> -->
                            <Button :label="showDetails ? 'Hide Details' : 'View Details'" severity="secondary" class="btn-sm" @click="showDetails = !showDetails"></Button>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td colspan="5" class="p-0">
                        <div class="details-holder grid grid-cols-3">
                            ...
                            <div class="dh-item item-details">
                                ...
                                <div class="dh-content">
                                    <ul>
                                        <li>Description : <b>{{ auctionDetails.description }}</b></li>
                                        <li>CurrentRound : <b>{{ auctionDetails.currentRound }}</b></li>
                                        <li>StartValue : <b>{{ auctionDetails.startValue }}</b></li>
                                        <li>ModifierValue : <b>{{ auctionDetails.modifierValue }}</b></li>
                                        <li>MaxBidAmount : <b>{{ auctionDetails.maxBidAmount }}</b></li>
                                ...
                                <div class="dh-title flex gap-2 items-center justify-between">
                                    ...
                                    <div class="dh-title-info flex gap-2.5 items-center">
                                        <div class="dh-ti-item flex gap-1 items-center">
                                            <div class="dh-ti-shape w-4 h-4 border border-emerald-700 rounded-[3px] bg-emerald-700 bg-opacity-40"></div>
                                            <div class="dh-ti-text text-sm font-normal text-[var(--neutral-200)]">Sold</div>
                                        </div>
                                        <div class="dh-ti-item flex gap-1 items-center">
                                            <div class="dh-ti-shape w-4 h-4 border border-rose-700 rounded-[3px] bg-rose-700 bg-opacity-40"></div>
                                            <div class="dh-ti-text text-sm font-normal text-[var(--neutral-200)]">Unsold</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="dh-content">
                                    <div class="dh-tags flex flex-wrap gap-1 items-center">
                                        <span v-for="item in properties" :key="item.fklInventoryId" class="px-2 py-1 rounded text-sm font-medium bg-opacity-20" :class="{ 'text-emerald-700 bg-emerald-700': item.bInventorySold, 'text-rose-700 bg-rose-700': !item.bInventorySold }">{{ item.vsInventoryName }}</span>
                                    </div>
        ...
        <Card class="mt-5 min-w-min max-w-full hidden">
            <template #header>
                <div class=" pt-4 flex justify-evenly">
                    <p class="text-xs font-bold">Auction Code: {{ auctionDetails.auctionCode }}</p>
                    <p class="text-xs font-bold">Auction Id: {{ auctionDetails.auctionId }}</p>
                    <p class="text-xs font-bold">Round: {{ auctionDetails.currentRound }} of {{ auctionDetails.numberOfRounds }}</p>
                    <Button label="Leave Auction" class="max-w-min" @click="leaveAuction"></Button>
                </div>
            </template>
            <template #content>
                <!-- Table -->
                ...
                <Accordion @update:activeIndex="activeIndexOpen" :activeIndex="isAuctionActive">
                    <AccordionTab header="Place Bid">
                        <template #default>
                            <div class="flex align-items-center gap-2">
                                <div class="flex flex-col">
                                    <span> Modifier value : {{ auctionDetails.modifierValue }} </span>
                                    <span class="font-black" v-if="bidFlag && !roundHasEnded && count">
                                        {{ currencyFormat(bidValue) }}
                                    </span>
                                </div>
                                <span v-if="bidFlag && !roundHasEnded">
                                    <Dropdown v-model="count" :options="multiplieries" optionLabel="text"
                                        optionValue="value" placeholder="Multiplier" class="w-full md:w-14rem" />
                                </span>
                            ...
                            <TabView>
                                    ...
                                    <template #default>
                                        <div class="max-h-36 overflow-y-scroll">
                                            <ul>
                                                <li v-for="(copyData, index) in bidHistory" :key="index">
                                                    Round : <strong>{{ copyData.roundNumber }}</strong>
                                                    Amt: <strong class="my-2">{{ currencyFormat(copyData.quoteAmount)
                                                        }}</strong>
                                                    @ <strong>{{ copyData.quoteTime }}</strong>
                                                </li>
                                            </ul>
                                    ...
                                    <template #default>
                                        <div class="max-h-36 overflow-y-scroll">
                                            <ul>
                                                ...
                                                <li>ModifierValue : <b>{{ auctionDetails.modifierValue }}</b></li>
                                                <li>MaxBidAmount : <b>{{ auctionDetails.maxBidAmount }}</b></li>
                                                <li>TotalNoOfBids : <b>{{ auctionDetails.totalNoOfBids }}</b></li>
                                            </ul>
                                        </div>
                                    </template>
                                </TabPanel>
                                <TabPanel header="Property Details">
                                    <template #default>
                                        <div class="max-h-36 overflow-y-scroll">
                                            <p>{{ properties[0]?.vsInventoryCategoryName }}:{{
                                                properties[0]?.inventoryHierarchy }}</p>
                                            <DataTable showGridlines :value="properties">
                                                <Column field="vsInventoryName" header="Property Name"></Column>
                                                <Column field="bInventorySold" header="Status">
                                                    <template #body="row">
                                                        <p
                                                            :class="{ 'font-bold text-green-600': row.data.bInventorySold, 'font-bold text-red-600': !row.data.bInventorySold }">
                                                            {{ row.data.bInventorySold ? 'Sold' : 'Unsold' }}</p>
                                                    </template>
                                                </Column>
                                            </DataTable>
                                        </div>
...
function bidPlaced() {
    bidFlag.value = false
    let totalvalue = parseInt(bidValue.value) + parseInt(auctionDetails.value.currentHigh)
...
const roundHasStarted = computed(() => moment(auctionDetails.value.roundStartTime).isSameOrBefore(moment(props.latestTime, "DD/MM/YYYY hh:mm:ss A"), 'second'));
const roundHasEnded = computed(() => moment(auctionDetails.value.roundEndTime).isSameOrBefore(moment(props.latestTime, "DD/MM/YYYY hh:mm:ss A"), 'second'));
...
function leaveAuctionFinal() {
    ...
    toast.add({ severity: 'success', summary: 'Auction Left', detail: `Left Auction ${auctionDetails.value.auctionCode} Successfully`, life: 3000 })
...
function auctionLeavingFunctionality() {
    ...
    console.log("aucId auctionLeavingFunctionality",auctionDetails.value.auctionId)
...
const openItemSelection = computed(() => {
    if (roundHasEnded.value == true && isHighestBidder.value == true && isItemSelectionBtnDisable.value == true && moment(props.latestTime, "DD/MM/YYYY hh:mm:ss A").isSameOrAfter(moment(auctionDetails.value.roundEndTime), 'second')) {
        return true
    } else {
        return false
    }
...
function submitProperty() {
    ...
    if (selectedCategory.value !== '') {
        wsConnection.value.send(
            ...
            let propertyName = properties.value.find((p) => p.pklAuctionItemsDetails == selectedCategory.value)
        toast.add({ severity: 'success', summary: 'Congratulation', detail: `Selected Property ${propertyName.vsInventoryName} is now allocated to you.`, life: 3000 })
...
function webSocketConn() {
    wsConnection.value = new WebSocket(`wss://${window.location.host}/bidding-server-ws/ws/auction`)
    ...
    wsConnection.value.addEventListener('message', function (e) {
        ...
        switch (true) { // Change this to status based code
            ...
            case message.typeCode === 400:
                ...
                if (message.bidderId == loginStore.loginId && isPropertySelected.value == true) {
                    isPropertySelected.value=false;
                    // alert(`Congratulations the selected property has been sold to you.`)
                }else if(message.bidderId == loginStore.loginId && isPropertySelected.value == false){
            ...
            case message.typeCode === 500:
                ...
                // self.leaveAuction()
                alert(`${auctionDetails.value.auctionCode} Auction Ended, Thank you for Participating!`)
            ...
            case message.typeCode === 600:
                alert(`The current ${auctionDetails.value.auctionCode} auction has been cancelled abruptly by the department due to unavoidable circumstances. We regret for the inconvenience caused.`)
                // self.$router.push({ name: 'vendorDashboard' })
                // window.close()
                auctionLeavingFunctionality()
                break
            case message.typeCode === 700:
                alert(` ${auctionDetails.value.auctionCode} Auction Ended, As all items for the auction is sold. Thank you for Participating!`)
                // self.$router.push({ name: 'vendorDashboard' })
                // window.close()
                auctionLeavingFunctionality()
                break
...
async function updateAuctionTimeLeft() {
    ...
    var msg;
    var finalMsg
    remaningTime.value = roundHasStarted.value ? moment(auctionDetails.value.roundEndTime).diff(moment(props.latestTime, 'DD/MM/YYYY hh:mm:ss A')) : moment(auctionDetails.value.roundStartTime).diff(moment(props.latestTime, 'DD/MM/YYYY hh:mm:ss A'));

    itemSelectionTimeLeft.value = moment(auctionDetails.value.roundEndTime).add(auctionDetails.value.itemSelectionTime, 'm').diff(moment(props.latestTime, 'DD/MM/YYYY hh:mm:ss A'))
    ...
    if (!roundHasStarted.value && !roundHasEnded.value) {
        ...
        if (moment.duration(remaningTime.value).hours()) {
            finalMsg = `${msg} <b class='text-red-600 font-bold'>${moment.duration(remaningTime.value).hours()} hrs:${moment.duration(remaningTime.value).minutes()} mins:${moment.duration(remaningTime.value).seconds()} secs</b>`;
        } else {
            finalMsg = `${msg} <b class='text-red-600 font-bold'>${moment.duration(remaningTime.value).minutes()} mins:${moment.duration(remaningTime.value).seconds()} secs </b>`;
        }
        ...
        if (auctionDetails.value.currentRound == auctionDetails.value.numberOfRounds) {
...
function totalBid() {
    // bidFlag.value = true
    bidValue.value = parseInt(auctionDetails.value.modifierValue) * parseInt(count.value);
}
        ...
        new MQL()
            ...
            .then(async (rs) => {
                ...
                if (rs.isValid("FetchDataForBidding")) {
                    ...
                    auctionDetails.value.auctionCode = res.result.inventoryDetails.auctionCode
                    auctionDetails.value.description = res.result.inventoryDetails.auctionDescription
                    auctionDetails.value.currentRound = res.result.inventoryDetails.currentRoundNumber
                    auctionDetails.value.startValue = res.result.inventoryDetails.startValue
                    auctionDetails.value.maxBidAmount = 100 * res.result.inventoryDetails.modifierValue
                    auctionDetails.value.totalNoOfBids = res.result.auctionHistory.length
                    auctionDetails.value.numberOfRounds = res.result.inventoryDetails.numberOfRounds
                    auctionDetails.value.itemSelectionTime = parseInt(res.result.inventoryDetails.itemSelectionTime)
                    auctionDetails.value.roundStartTime = res.result.inventoryDetails.currentRoundStartTime
                    auctionDetails.value.roundEndTime = res.result.inventoryDetails.currentRoundEndTime
                    auctionDetails.value.currentHigh = res.result.inventoryDetails.currentHigh
                    auctionDetails.value.modifierValue = res.result.inventoryDetails.modifierValue
                    auctionDetails.value.roundNumber = res.result.inventoryDetails.currentRoundNumber
                    ...
                    auctionDetails.value.currentHigh = res.result.inventoryDetails.startValue
                    }
